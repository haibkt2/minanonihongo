...
0 != lastFingerprint && lastFingerprint != fingerprint && (console.log("You are kicked out"), $.get(window.location.origin + "/account/logout", function(t, e) {}), $.fancybox.open('<div class="message" id="kick-popup"><h3 style="color:#e74c3c;"><i class="zmdi zmdi-alert-triangle"></i> Cảnh báo!</h3><p>Bạn bị <b>đăng xuất</b> do hệ thống của chúng tôi phát hiện ra một phiên làm việc khác <br/> của tài khoản này đang chạy trên trình duyệt <b>' + lastBrowser + "</b> sử dụng hệ điều hành <b>" + lastOS + '</b></p><p style="font-size: 12px;">* Lưu ý : Điều khoản sử dụng của website dungmori.com không cho phép <br/>người dùng đăng nhập đồng thời trên nhiều thiết bị</p><p>Vui lòng <b>đăng nhập lại</b> để có thể tiếp tục sử dụng<br/>khi đó tất cả phiên làm việc trên các thiết bị khác sẽ bị đăng xuất</p><div class="pull-right dmr-btn" onclick="clickClosePopup()" data-fancybox-close>Đồng ý</div></div>'));