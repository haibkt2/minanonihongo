...
public interface JLPTMenuRepository extends CrudRepository<JLPTMenu, Integer> {
	@Query(value="SELECT jm.jlpt_menu_id, jm.jlpt_menu_name FROM jlpt_menu as jm JOIN jlpt as j on jm.jlpt_menu_id = j.jlpt_menu_id WHERE j.course_id = ?1 AND j.jlpt_type_id = ?2",nativeQuery = true)
